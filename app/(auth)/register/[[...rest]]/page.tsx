...
export default function RegisterPage() {
  const { t } = useT()
  const highlights = [t.profiles.addProfile, t.profiles.manageProfiles, t.nav.watchlist, t.nav.history, t.nav.movies, t.nav.series]

  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden px-4 py-8 md:py-12">
      <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(to bottom right, #0A0A0A, #141414, #0A0A0A)' }} />
      <div
        className="absolute inset-0 opacity-20"
        style={{
          backgroundImage:
            'radial-gradient(circle at 80% 50%, #E50914 0%, transparent 50%), radial-gradient(circle at 20% 80%, #0071E3 0%, transparent 40%)',
        }}
      />

      <div className="relative z-10 w-full max-w-6xl grid gap-6 lg:grid-cols-[0.95fr_1.05fr] items-stretch">
        <motion.aside
          {...slideUp}
        ...
        >
          <div className="absolute inset-0 opacity-50" style={{ backgroundImage: 'radial-gradient(circle at top right, rgba(229,9,20,0.18), transparent 38%), radial-gradient(circle at bottom left, rgba(0,113,227,0.14), transparent 35%)' }} />
          <div className="relative z-10">
            ...
            <div className="mt-10 max-w-lg">
              <h1 className="text-5xl font-black tracking-tight text-white leading-tight">{t.auth.register}</h1>
          ...
          <div className="relative z-10 grid grid-cols-2 gap-3 text-sm text-[#B3B3B3]">
            <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
              <div className="text-white font-semibold">{t.profiles.addProfile}</div>
              <div className="mt-1">{t.profiles.manageProfiles}</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/25 p-4">
        ...
        <motion.section {...slideUp} className="w-full max-w-xl mx-auto lg:mx-0 lg:justify-self-start">
